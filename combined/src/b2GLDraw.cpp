...
void b2GLDraw::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color) {
	...
	for (int i = 0; i < vertexCount; i++) {
		b2Vec2 v = vertices[i];
		glVertex2f(v.x * 30 * SCALE1, v.y * 30 * SCALE1);
	}
	glEnd();
}
...
void b2GLDraw::DrawSegment(const b2Vec2& p1, const b2Vec2& p2, const b2Color& color) {
	...
	glBegin(GL_LINES);
	glVertex2f(v1.x * 30 * SCALE1, v1.y * 30 * SCALE1);
	glVertex2f(v2.x * 30 * SCALE1, v2.y * 30 * SCALE1);
	glEnd();
}
...
void b2GLDraw::DrawAABB(b2AABB* aabb, const b2Color& c) {
	...
	glBegin(GL_LINE_LOOP);
	glVertex2f(aabb->lowerBound.x * 30 * SCALE1, aabb->lowerBound.y * 30 * SCALE1);
	glVertex2f(aabb->upperBound.x * 30 * SCALE1, aabb->lowerBound.y * 30 * SCALE1);
	glVertex2f(aabb->upperBound.x * 30 * SCALE1, aabb->upperBound.y * 30 * SCALE1);
	glVertex2f(aabb->lowerBound.x * 30 * SCALE1, aabb->upperBound.y * 30 * SCALE1);
	glEnd();
}
...
void b2GLDraw::DrawTransform(const b2Transform& xf) {
	...
	b2Vec2 p1 = xf.p, p2;
	const float k_axisSCALE1 = 0.0f;

	p2 = p1 + k_axisSCALE1 * xf.q.GetXAxis();
	DrawSegment(p1 * SCALE1, p2 * SCALE1, b2Color(1, 0, 0));
	p2 = p1 + k_axisSCALE1 * xf.q.GetYAxis();
	DrawSegment(p1 * SCALE1, p2 * SCALE1, b2Color(0, 1, 0));
}
