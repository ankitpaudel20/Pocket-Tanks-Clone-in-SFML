...
terrain::terrain(std::vector<unsigned int>& value, sf::RenderWindow& window) {
	...
	for (int w = 0; w < window_size.x; w++) {
		...
		for (int h = 0; h < window_size.y - value[w]; h++) {
			...
			fills[w][h].color = sf::Color(static_cast<float>(255) / (window_size.y - lowest) * h,/* static_cast<float>(255) / window_size.y * h , static_cast<float>(255) / (window_size.y - lowest) * h */ 255, 255);
		}
